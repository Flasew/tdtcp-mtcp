...
SendUntilAvailable(struct thread_context *ctx, int sockid, struct server_vars *sv)
{
  ...
  while (ret > 0) {
    len = MIN(SNDBUF_SIZE, 
      (sv->perf_mode ? sv->requested_len : sv->fsize) - sv->total_sent);
    if (len <= 0) {
      break;
    }
    if (sv->perf_mode) {
      ret = mtcp_write(ctx->mctx, sockid, perf_buf, len);
    }
    else {
  ...
  if ((sv->perf_mode  && sv->total_sent >= sv->requested_len) ||
      (!sv->perf_mode && sv->total_sent >= fcache[sv->fidx].size)) {
...
HandleReadEvent(struct thread_context *ctx, int sockid, struct server_vars *sv)
{
  ...
  sprintf(response, "HTTP/1.1 %d %s\r\n"
      ...
      "Content-Length: %ld\r\n"
      "Connection: %s\r\n\r\n", 
      scode, StatusCodeToString(scode), t_str, 
      (sv->perf_mode ? sv->requested_len : sv->fsize), 
      keepalive_str);
  len = strlen(response);
