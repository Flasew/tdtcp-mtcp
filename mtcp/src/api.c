...
mtcp_setsock_block(mctx_t mctx, int sockid)
{
  ...
  mtcp->smap[sockid].opts &= ~((int) MTCP_NONBLOCK);
...
#if TDTCP_ENABLED
  uint32_t cur_head = rcvvar->rcvbuf->head_seq;
  struct tdtcp_seq2subflow_map * tnodes2s = 
      (struct tdtcp_seq2subflow_map *)(rbt_leftmost(cur_stream->rx_seq_subflow_map));
    // DELETE UP TO
    while (tnodes2s && (tnodes2s->dsn) < cur_head) {
