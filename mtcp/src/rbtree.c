...
rbt_create(size_t node_size,
           ...
           void *arg)
{
    RBTree     *tree = (RBTree *) calloc(sizeof(RBTree), 1);
...
/*
 * rbt_rightmost: fetch the rightmost (biggest-valued) tree node.
 * Returns NULL if tree is empty.
 *
 * Note: in the original implementation this included an unlink step, but
 * that's a bit awkward.  Just call rbt_delete on the result if that's what
 ...
 */
RBTNode *
rbt_rightmost(RBTree *rbt)
{
    RBTNode    *node = rbt->root;
    RBTNode    *rightmost = rbt->root;
...
rbt_begin_iterate(RBTree *rbt, RBTOrderControl ctrl, RBTreeIterator *iter)
{
    ...
    switch (ctrl)
    {
        ...
        case RightLeftWalk:     /* visit right, then self, then left */
            ...
            break;
        default:
            TRACE_ERROR("unrecognized rbtree iteration order: %d", ctrl);
    }
}
...
void rbt_free(RBTree * tree) {
    RBTNode * tnode = rbt_leftmost(tree);
    // DELETE ALL
    while (tnode) {
      rbt_delete(tree, tnode);
      tnode = rbt_leftmost(tree);
    }
    free(tree);
}