    ...
    const struct tcphdr *tcph, uint32_t seq, uint16_t window)
{
  ...
  for (int x = 0; x < cur_stream->td_nrxsubflows; x++) {
    ...
    cur_stream->rx_subflows[x].rcvbuf = calloc(sizeof(tdtcp_rx_rcvbuf), 1);
  ...
  cur_stream->tx_seq_subflow_map = rbt_create(sizeof(struct tdtcp_seq2subflow_map), 
                               &tdtcp_seq2subflow_comp, 
                               &tdtcp_seq2subflow_comb,
                               &tdtcp_seq2subflow_alloc,
                               &tdtcp_seq2subflow_free,
                               NULL);
  cur_stream->rx_seq_subflow_map = rbt_create(sizeof(struct tdtcp_seq2subflow_map), 
                               &tdtcp_seq2subflow_comp, 
                               &tdtcp_seq2subflow_comb,
                               &tdtcp_seq2subflow_alloc,
                               &tdtcp_seq2subflow_free,
...
// #endif
  ...
  if (rmlen > 0) {
    ...
    struct tdtcp_seq2subflow_map * tnodes2s = 
      (struct tdtcp_seq2subflow_map *)(rbt_leftmost(cur_stream->tx_seq_subflow_map));
    // DELETE UP TO
